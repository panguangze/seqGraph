...
for line in inp2:
    ...
    if vc in tmp.keys() and kc in tmp[vc]:
        ...
        # write_juncs.append(" ".join(vs)+"\n")
        tmp[vc].remove(kc)
    # if vs[1] in blast_segs or vs[3] in blast_segs or vs[1] in gene_res or vs[3] in gene_res or vs[1] == vs[3] or (
    # float(vs[-1]) >= float(depth)/2 and (vs[1] in relevate_blast_segs or vs[3] in relevate_blast_segs) and get_len(
    # vs[1]) <= relevate_edge_len and get_len(vs[3]) <= relevate_edge_len) or (left_score>=0.7 and right_score>=0.7):

    if (vs[1] in blast_segs and vs[3] in blast_segs) or (vs[1] in gene_res and vs[3] in gene_res) or (
...
for item in tmp.keys():
    ...
    for i in tmp[item]:
        ...
        # first not in blast_segs and second not in blast_segs and first not in gene_res and second not in gene_res
        # and first != second) and ( (first not in relevate_blast_segs or second not in relevate_blast_segs) or (
        # first in relevate_blast_segs and second in relevate_blast_segs and get_len( first) <= relevate_edge_len and
        # get_len(second) <= relevate_edge_len)) and ( left_score < 0.7 or right_score < 0.7): print(first,
        # left_score, second, right_score)
        ...
        if ((left_score < 0.2 and left_node_len > 10000) or (right_score < 0.2 and right_node_len > 10000)) or \
                ...
                left_score < 0.6 or right_score < 0.6):
            continue
